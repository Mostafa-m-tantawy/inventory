...
$(document).ready(function (e) {
    ...
    $("#unitPrice ,#quantity ,#percentage_discount").on('keyup wheel', function (e) {
        ...
        var vat=$("#vat").val()? (( $("#unitPrice").val()-discount) * ($("#vat").val()/100)):0

        console.log(discount)
        $("#grossPrice").val($("#quantity").val() * $("#unitPrice").val()- discount*$("#quantity").val()+ vat*$("#quantity").val() );
        $("#child_unit_price").val( ( ($("#unitPrice").val()-discount +vat)/  convertion_rate).toFixed(2));
    });
    /**
    ...
    $("#product").on('change', function (e) {
        ...
        $("#vat_value").val($('#product option:selected ').data('vat'));
