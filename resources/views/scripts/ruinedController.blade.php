    ...
    $("#type").on('change', function () {
        ...
        if (purses.length != 0) {
            if (clicked_ruined_type != '') {
                ...
                toastr.error('Cannot change  type!')
            ...
            if ($(this).val() != '') {
                ...
                $.ajax({
                    url: '{{url('stock/get-assignable-ruined')}}/' + $(this).val(),
                    type: "POST",
                    data: formdata,
                    processData: false,
                    contentType: false,
                    success: function (data) {
                        ...
                        if (clicked_ruined_type == 'restaurant') {
                            $('#from_stock').html('');
                            $('<option ></option>').val('').text('{{trans('main.select one')}}').appendTo('#from_stock');
                    ...
                    error: function (data) {
                        if (data['status'] == 422) {
                            toastr.error('There is no list fro selection!')
...
// console.log(purses)
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#pursesDetailsRender").empty();
                $(this).renderHtml(purses);
                document.location.href = '{{url('stock/ruined')}}';
